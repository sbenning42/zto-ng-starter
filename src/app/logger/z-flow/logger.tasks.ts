...
export class LoggerTaskLog extends ZFlowTask {
    ...
    execute(requires: ZDictionnary): Observable<ZDictionnary> {
        return this.injector.loggerService.log(...requires[LoggerSymbol.logMessages]).pipe(map(provideNothing));
    }
}
export class LoggerTaskError extends ZFlowTask {
    ...
    execute(requires: ZDictionnary): Observable<ZDictionnary> {
        return this.injector.loggerService.error(...requires[LoggerSymbol.errorMessages]).pipe(map(provideNothing));
    }
}
